...
class PurchaseOrder(models.Model):
    ...
    def action_view_invoice(self, invoices=False):
        ...
        # Some migrated/custom action records may have empty or missing context.
            # 🔥 BULLETPROOF CONTEXT FIX
        if result.get('type') == 'ir.actions.act_window':
            ...
            if isinstance(ctx, dict):
                ...
                try:
                    result['context'] = literal_eval(ctx)
                except Exception:
                    result['context'] = {}
            else:
                result['context'] = {}
        ...
        # result['context'] = literal_eval(result.get('context') or "{}")
        # if len(self.partner_id) == 1:
        #     result['context']['default_partner_id'] = self.partner_id.id
        return result
