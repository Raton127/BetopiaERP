...
class Session(http.Controller):
    ...
    @http.route(['/web/session/get_session_info', APP_WEB_ROOT + '/session/get_session_info'], type='jsonrpc', auth='user', readonly=True)
    def get_session_info(self):
        # Crapy workaround for unupdatable Odoo Mobile App iOS (Thanks Apple :@)
        request.session.touch()
        return request.env['ir.http'].session_info()

    @http.route(['/web/session/authenticate', APP_WEB_ROOT + '/session/authenticate'], type='jsonrpc', auth="none", readonly=False)
    ...
    @http.route(['/web/session/get_lang_list', APP_WEB_ROOT + '/session/get_lang_list'], type='jsonrpc', auth="none")
    ...
    @http.route(['/web/session/modules', APP_WEB_ROOT + '/session/modules'], type='jsonrpc', auth='user', readonly=True)
    def modules(self):
        # return all installed modules. Web client is smart enough to not load a module twice
        return list(request.env.registry._init_modules)

    @http.route(['/web/session/check', APP_WEB_ROOT + '/session/check'], type='jsonrpc', auth='user', readonly=True)
    def check(self):
        return  # ir.http@_authenticate does the job

    @http.route(['/web/session/account', APP_WEB_ROOT + '/session/account'], type='jsonrpc', auth='user', readonly=True)
    def account(self):
    ...
    @http.route(['/web/session/destroy', APP_WEB_ROOT + '/session/destroy'], type='jsonrpc', auth='user', readonly=True)
    def destroy(self):
        request.session.logout()

    @http.route(['/web/session/logout', APP_WEB_ROOT + '/session/logout'], type='http', auth='none', readonly=True)
    def logout(self, redirect=APP_ROOT):
        request.session.logout(keep_db=True)
        if redirect.startswith('/web/'):
            redirect = APP_WEB_ROOT + redirect.removeprefix('/web')
        elif redirect == '/odoo':
            redirect = APP_ROOT
